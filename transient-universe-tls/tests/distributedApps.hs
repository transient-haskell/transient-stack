...
-- set -e && port=`echo ${3} | awk -F/ '{print $(3)}'` && docker run -it -p ${port}:${port} -v $(pwd):/work agocorona/transient:05-02-2017  bash -c "cd /work && mkdir -p static && ghcjs ${1} -o static/out && ghc ${1} && ./$(basename $1 .hs)${2} ${3}"
...
main = do
  initTLS
  keep $  initNode  $ menuApp  <|> thelink <|> inputNodes
