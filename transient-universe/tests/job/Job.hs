...
restartable  jobname job= do
    ...
    restorable job=  do
     return () !> "RESTORABLE"
     r <- (Right <$> restore' job) `catchc` \(e :: SomeException) -> return . Left $ show e
     case r of
       Left err -> do
...
main7= keep  . initNode $ inputNodes <|> process
    where
   
    process= do
        local $ option "g"  "execute"
...
main9= keep $ initNode $ inputNodes <|> do 
    ...
    where
    delay = local $  do
        -- option "r"  "respond" -- threads 0 $ choose [1..10:: Int]
...
main6= keep $ do
    r <- Transient $ do
        runTrans $ option "g" "go" -- onException $ \(e:: SomeException) -> empty 
...
main5= keep $ initNode $ inputNodes <|> do
    local $ option "g" "go"
    nodes <- local getNodes
    r <- (,) <$> (runAt (nodes !! 1)$ local $ option "h"  "hello") <*> (runAt (nodes !! 1)$ local $ option "w" " world")
    localIO $ print ("----------->",r)
   {- 
    process1=   do
         requires ["ls","docker"]
