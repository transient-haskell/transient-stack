...
-- elemById :: (MonadState TranShip m, MonadIO m) => JSString -> m (Maybe Elem)
-- #ifdef ghcjs_HOST_OS
-- elemById id = do
--   mid <- getData
--   case mid of
...
--       -- alert ("elemById",id1, id)
--       liftIO $ do
--           let id2= JS.takeWhile (/='p') id
--           re <- elemByIdDOM id1 id2
--           fromJSVal re
-- #else
...
-- newtype Widget a=  Widget{ norender :: TransIO a} deriving(MonadIO, Applicative, Alternative, MonadState TranShip,MonadPlus,Num)
...
wcallback x f= do
    nid <-  genNewId
    prev <- getData `onNothing` return noHtml
   
    delData noHtml
    x <- at nid Insert x
    r <- at nid Insert $ f x
    rend <- getData `onNothing` return noHtml
    setData $ prev <> (span ! atr "id" nid $ noHtml) <> rend
    return r
...
getParam1 exact  par = do
  ...
  --  if isTemplate then return NoParam else do
       ...
       me <- if exact then elemById par else elemById par
...
getParamS look type1 mvalue=  do
    tolook <- case look of
       Nothing  -> getData `onNothing` error "no event id" -- genNewId
       Just n -> return n
...
clearScreen=  do
   wraw $ forElems "body" $ this  >> clear `child` (div ! atr "id" "body1" $ noHtml)
  --  setRenderTag "body1"
...
wlink x v=  do
    ...
    Path paths <- getSData <|> return (Path  [])
...
raiseEvent w event =  do
   ...
   Transient $ do
      ...
      --  else do
       do
          me <- elemById id'                                          --  !> ("adding event to",  id')
          case me of
...
runBody ac= do
   body <- getBody
   runTransient $ runWidget' ac body 
...
--                 meid2 <- liftIO $ elemByIdDOM id1 id2 >>= fromJSVal  -- elemById id2
